...
bool qb_force::readMeasCurrent() {
    ...
    for(int i=0;i<hand_chain_.size();i++){
        ...
        if (p_meas[0]!=0) {
            ...
            speed_=(p_meas[0]-pos_.at(0))/(now-last_).toSec();
            pos_.clear();
...
void qb_force::move(float &debug_ref) {
    ...
    if (pos_ref_.size()>0) {
        ...
        if(pos_ref_.size()>1){
            des_cur+=pos_ref_.at(1);
        }
        if(pos_.at(0)>19500 && des_cur>0 ) des_cur=0; //avoid winding the tendon the wrong way around
        if(pos_.at(0)<100 && des_cur<0 ) des_cur=0; //avoid winding the tendon the wrong way around

        ROS_WARN_THROTTLE(0.5,"POS REF: %f POS: %f CURR: %f SPEED: %f",pos_ref_.at(0),pos_.at(0),des_cur,speed_);

        //meas[0] = (short) pos_ref_.at(0);
        //meas[1] = (short) pos_ref_.at(0);
        if(des_cur>1500) des_cur=1500;
        if(des_cur<-1500) des_cur=-1500;
