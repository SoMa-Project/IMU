...
#define S_FUNCTION_NAME  qbgetImuReadings_v1
...
//============================================================      qbot ID check
	...
	//	printf("IMU: %d\n", imu_table[2*i + 0]);
		...
		for (int k=0; k<3; k++)
			out(i)[k] = out(i)[k] * meas_unity_acc;
		for (int k=3; k<6; k++)
			out(i)[k] = out(i)[k] * meas_unity_gyro;
		for (int k=6; k<9; k++)
			out(i)[k] = out(i)[k] * meas_unity_mag;
